...
type GetAllBlobsParams struct {
	Limit  int64
	Offset int64
}

func (q *Queries) GetAllBlobs(ctx context.Context, arg GetAllBlobsParams) ([]Blob, error) {
	rows, err := q.db.QueryContext(ctx, getAllBlobs, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getTotalBlobsCount = `-- name: GetTotalBlobsCount :one
select count(*) as total
from blobs
`

func (q *Queries) GetTotalBlobsCount(ctx context.Context) (int64, error) {
	row := q.db.QueryRowContext(ctx, getTotalBlobsCount)
	var total int64
	err := row.Scan(&total)
	return total, err
}
